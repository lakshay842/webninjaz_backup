 ...
 //  ==Define here cause we can call the resetAllstate() call globaly==//
  ...
  const resetAllstate = () => {
    ...
    setShowpriceform(false);
      ...
      var monitorFormElement = document.querySelector('.monitor-form');
      if (monitorFormElement) {
        sessionStorage.setItem('active_val', monitorFormElement.id);
      }

      // Retrieve a value from sessionStorage
      var retrievedValue = sessionStorage.getItem('active_val');
      if(retrievedValue != '' ){

        console.log('check123');

        var parts = retrievedValue.split("-")[1];


        setTimeout(() => {
          // it's expanding filter
          document.getElementById('filter-sub-id').click();
         
        }, 100);

        setTimeout(() => {
          // after expand clicked apply button
          document.getElementById(parts).nextElementSibling.click();
          document.getElementById('filter-aply-btn').click();
        }, 100);

        // clear a value in sessionStorage
        sessionStorage.setItem('active_val', '');
      }else{
        ...
        setTimeout(() => {
          // after expand clicked reset button
    ...
    return <>
      <div className="main-learn-div">
        ...
        <div className="learn-cnt">
          ...
          <p>By creating Alerts, you can receive important notifications about various types of blockchain transactions & activity, so you can
            maintain better control over your digital assets, and always stay on top of market movements and trends. There are three types
            of alerts, Personal Monitor, Community Monitor and Price Alert.</p>
        </div>
